...
function GameServer(lobby) {
    ...
    // main player of the game, HERO 
    this.hero = 0
    // array to store ids of players with shield power
    this.powerShieldPlayers = [];
...
GameServer.prototype.start = function(configuration) {
    console.log('[Game] Starting new game room');
    ...
    this.startingCoins();
    // Start Main Loop
    this.loopId = setInterval(this.mainLoop.bind(this), 1);
...
GameServer.prototype.checkPlayersAmount = function() {
    "use strict";
    if (this.clients.length <= this.config.serverBots) {
        this.stop();

        var index = this.lobby.rooms.indexOf(this);
        if (index != -1) {
            console.log('[Game] Remove gameroom with index ' + index);
            this.lobby.rooms.splice(index,1);
        }
    }
}

GameServer.prototype.addPlayer = function(ws) {
    "use strict";
...
GameServer.prototype.removeNode = function(node) {
 ...
 this.checkShieldedPlayers()

    // if node has shiled power dont remove it
    if( this.powerShieldPlayers.indexOf(node.nodeId) != -1 )  {

        console.log("=========================================================================");
        console.log("=========================================================================");
        console.log("Game server removeNode cant remove node with shield   where node ID is"+node.nodeId);
        console.log("=========================================================================");
        console.log("=========================================================================");
        return;
    }

    //console.log(" ------- [Game Server] remove node  ------")
...
// Coin spawing
GameServer.prototype.startingCoins = function() {
    // Spawns the starting amount of food cells
    for (var i = 0; i < this.config.coinsStartAmount; i++) {
        this.spawnCoin();
    }
};

GameServer.prototype.updateCoins = function() {
    var toSpawn = Math.min(this.config.coinsSpawnAmount,(this.config.coinsMaxAmount-this.currentCoins));
    for (var i = 0; i < toSpawn; i++) {
        this.spawnCoin();
    }
};

GameServer.prototype.spawnCoin = function() {
    var f = new Entity.Coin(this.getNextNodeId(), null, this.getRandomPosition(), this.config.coinMass);
    f.setColor({
        r: 0,
        b: 0,
        g: 0
...
GameServer.prototype.spawnPlayer = function(player,pos,mass) {
    ...
    console.log(" [ GameServer] ---------- spawn Player  getNextNodeId: "+ nodeId);

    var cell = new Entity.PlayerCell(nodeId, player, pos, mass);
    this.addNode(cell);
    //console.log(" [GameServer] spawnPlayer player added")
    // Set initial mouse coords

    player.mouse = {x: pos.x, y: pos.y};
    //console.log(" [Game Server ] spawn Player x"+ pos.x+"  y  "+pos.y)
...
GameServer.prototype.updateMoveEngine = function() {
    ...
    for (var i = 0; i < len; i++) {
        ...
        for (var j = 0; j < list.length ; j++) {
            ...
            //console.log(" ------------- check cell consumed ---------------")
            // Remove cell
...
GameServer.prototype.splitCells = function(client) {
    ...
    for (var i = 0; i < len; i++) {
        ...
        // Create cell
         ...
         console.log(" [ GameServer] getNextNodeId = "+ this.getNextNodeId())
...
GameServer.prototype.newCellVirused = function(client, parent, angle, mass, speed) {
    ...
    // Create cell
    console.log(" [ GameServer] newCell virused getNextNodeId "+ this.getNextNodeId())
...
GameServer.prototype.addShield = function(playerId) {
    console.log("######### GameServer adding shield, playerId = " + playerId)
    if( this.powerShieldPlayers.indexOf(playerId) == -1 ) {
        console.log("######### adding shield in array")
        this.powerShieldPlayers.push(playerId)
    }
};

GameServer.prototype.removeShield = function(playerId) {
    console.log("######### GameServer removing shield, playerId = " + playerId)

    if( this.powerShieldPlayers.indexOf(playerId) != -1) {
        console.log("######### removing shield from array")
        this.powerShieldPlayers.splice(this.powerShieldPlayers.indexOf(playerId),1);
    }
};

GameServer.prototype.checkShieldedPlayers = function(packet) {
    console.log("------------------ShieldedPlayers  are ----------------"+this.powerShieldPlayers)
}
