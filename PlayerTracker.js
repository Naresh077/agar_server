...
function PlayerTracker(gameServer, socket) {
    ...
    if (gameServer) {
        ...
        console.log("[PlayerTracker] pID: " + this.pID);
...
PlayerTracker.prototype.getCoins = function() {    
    var coinsTotal = 0;
    for (var i = 0; i < this.cells.length; i++) {
        coinsTotal += this.cells[i].coins;
        this.cells[i].coins = 0
    }
    return coinsTotal >> 0;
};
...
// Actions buffer (So that people cant spam packets)
    ...
    if (this.getCoins()>0) {
        this.socket.sendPacket(new Packet.CoinCollect(this.getCoins()));    
    }
    ...
    if (this.disconnect > -1) {
        ...
        if (this.disconnect == -1) {
            ...
            for (var i = 0; i < len; i++) {
                ...
                console.log(" [PlayerTracker] node removed")
            }
            ...
            if (index != -1) {
                ...
                this.gameServer.checkPlayersAmount()
            }
        }
    }
};
