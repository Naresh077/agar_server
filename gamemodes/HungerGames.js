...
HungerGames.prototype.onPlayerDeath = function(gameServer) {
    ...
    for (var i = 0; i < len; i++) {
        ...
        if (node.position.x < config.borderLeft) {
         console.log("  ------   HungerGames   onPlayerDeath removeNode ---------")

            gameServer.removeNode(node);
            i--;
        } else if (node.position.x > config.borderRight) {
         console.log("  ------   HungerGames   onPlayerDeath  borderRight removeNode ---------")

            gameServer.removeNode(node);
            i--;
        } else if (node.position.y < config.borderTop) {
            console.log("  ------   HungerGames   onPlayerDeath borderTop removeNode ---------")

            gameServer.removeNode(node);
            i--;
        } else if (node.position.y > config.borderBottom) {
            console.log("  ------   HungerGames   onPlayerDeath borderBottom removeNode ---------")
